...
##
## Usage:
##   bash start-pokeclaw-linux.sh            # full interactive onboarding
##   bash start-pokeclaw-linux.sh --quiet    # skip prompts, use env vars / defaults
##
...
# ── Step 1: System dependencies (curl, unzip) ─────────────────────────────────
PM="$(_detect_pm)"
...
if command -v cloudflared &>/dev/null; then
  ...
  case "$PM" in
    ...
    dnf)
      ...
      ;;
    pacman)
      # cloudflared is in AUR; attempt yay or paru
...
if [ "$QUIET" = false ]; then
  ...
  # Persist to ~/.bashrc?
  echo ""
  read -r -p "   Save settings to ~/.bashrc? [Y/n]: " _save
  _save="${_save:-Y}"
  if [[ "$_save" =~ ^[Yy]$ ]]; then
    # Remove old PokeClaw block if present
    if grep -q "# PokeClaw — start" "$HOME/.bashrc" 2>/dev/null; then
      sed -i '/# PokeClaw — start/,/# PokeClaw — end/d' "$HOME/.bashrc"
    fi
    {
      echo ""
      echo "# PokeClaw — start"
...
_existing=$(lsof -ti tcp:"${PORT}" 2>/dev/null || fuser "${PORT}/tcp" 2>/dev/null | awk '{print $1}' || true)
if [ -n "$_existing" ]; then
  echo ""
