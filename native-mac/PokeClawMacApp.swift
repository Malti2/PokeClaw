...
struct PokeClawMacApp: App {
    ...
    @AppStorage("pokeclaw.serverBunPath") private var serverBunPath = "bun"
    @AppStorage("pokeclaw.serverScriptPath") private var serverScriptPath = "~/pokeclaw/server.ts"
    @AppStorage("pokeclaw.serverRoots") private var serverRoots = "~"
...
struct PokeClawSettingsView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            await model.checkForUpdatesOnStartup()
        }
    ...
    private var serverPane: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Bun server")
                .font(.headline)
            Text("Start, stop, and configure the local Bun-based MCP server that ships with this repository.")
                .font(.caption)
                .foregroundStyle(.secondary)

            VStack(alignment: .leading, spacing: 12) {
                TextField("Bun executable", text: $serverBunPath)
                TextField("Server script", text: $serverScriptPath)
                TextField("Allowed roots", text: $serverRoots)
                SecureField("Auth token", text: $serverToken)
                TextField("Log level", text: $serverLogLevel)

                HStack(spacing: 10) {
                    Button(model.managedServerState == "Running" ? "Running" : "Start server") {
                ...
                HStack(spacing: 10) {
                    ...
                    if let latestURL = model.latestReleaseURL {
                        Button("Open Latest Release") {
                            NSWorkspace.shared.open(latestURL)
                        }
                        .buttonStyle(.bordered)
                    }
                }
...
final class PokeClawConnectionModel: ObservableObject {
    ...
    @Published var managedServerState: String = "Stopped"
    @Published var managedServerDetail: String = "Configure the Bun server path and press Start server."
    @Published var updateStatusText: String = "Checking for updates on startup…"
    @Published var latestReleaseURL: URL? = nil
    @Published var isCheckingForUpdates: Bool = false

    private var managedServerProcess: Process? = nil
    private var managedServerStandardOutput: Pipe? = nil
    private var managedServerStandardError: Pipe? = nil
    ...
    func checkForUpdatesOnStartup() async {
        guard latestReleaseURL == nil, !isCheckingForUpdates else { return }
    ...
    func checkForUpdates() async {
        ...
        let repoURL = URL(string: "https://api.github.com/repos/Malti2/PokeClaw/releases?per_page=20")!
        var request = URLRequest(url: repoURL)
        request.setValue("application/vnd.github+json", forHTTPHeaderField: "Accept")
        request.setValue("PokeClawMacApp/1.0", forHTTPHeaderField: "User-Agent")

        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
                updateStatusText = "Unable to check GitHub releases."
                return
            }

            let releases = try JSONDecoder().decode([GitHubRelease].self, from: data).filter { !$0.draft }
            guard let release = selectRelease(from: releases) else {
                updateStatusText = "No GitHub releases found."
                latestReleaseURL = nil
                return
            }

            latestReleaseURL = release.htmlURL
            if compareVersion(release.tagName, to: currentVersion) == .orderedDescending {
                let releaseName = release.name?.isEmpty == false ? release.name! : release.tagName
                updateStatusText = "Update available: \(releaseName)"
            } else {
                updateStatusText = "You are up to date (\(currentVersion))."
            }
        } catch {
    ...
    func startManagedServer() async {
        if let process = managedServerProcess, process.isRunning {
            managedServerState = "Running"
            managedServerDetail = "The Bun server is already running."
            return
        }

        let bunPath = configuredServerBunPath()
        let scriptPath = expandedPath(configuredServerScriptPath())
        let roots = configuredServerRoots()
        let token = configuredServerToken()
        let logLevel = configuredServerLogLevel()

        guard FileManager.default.fileExists(atPath: scriptPath) else {
            managedServerState = "Missing script"
            managedServerDetail = "Could not find \(scriptPath). Keep the Bun files in the repo and point the app to the script path."
            statusMessage = managedServerDetail
            return
        }
        ...
        process.arguments = ["-lc", "\(shellQuote(bunPath)) run \(shellQuote(scriptPath))"]
        process.standardOutput = stdoutPipe
        process.standardError = stderrPipe
        process.environment = [
            "POKECLAW_PORT": String(serverPort),
            "POKECLAW_ROOTS": roots,
            "POKECLAW_TOKEN": token,
            "POKECLAW_LOG_LEVEL": logLevel
        ]

        managedServerState = "Starting"
        managedServerDetail = "Launching Bun at \(bunPath) with \(scriptPath)"
        statusMessage = managedServerDetail
        lastAction = "Starting managed MCP server"
        ...
        stdoutPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            ...
            Task { @MainActor in
                self?.appendManagedServerOutput(text: text, stream: "stdout")
            }
        }
        stderrPipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            ...
            let text = String(decoding: data, as: UTF8.self)
            Task { @MainActor in
                self?.appendManagedServerOutput(text: text, stream: "stderr")
            }
        }
        ...
        do {
            ...
            managedServerStandardOutput = stdoutPipe
            managedServerStandardError = stderrPipe
            process.terminationHandler = { [weak self] _ in
                Task { @MainActor in
                    self?.managedServerDidExit()
                }
            }
            managedServerState = "Running"
            managedServerDetail = "Bun server is running from the app."
            statusMessage = managedServerDetail
        } catch {
            managedServerState = "Failed to start"
            managedServerDetail = error.localizedDescription
            statusMessage = managedServerDetail
            lastAction = "Managed server failed to start"
        }
    }

    func stopManagedServer() async {
        guard let process = managedServerProcess else {
            managedServerState = "Stopped"
            managedServerDetail = "The managed server is not running."
            return
        }

        managedServerState = "Stopping"
        managedServerDetail = "Requesting server shutdown…"
        lastAction = "Stopping managed MCP server"
        process.terminate()
        try? await Task.sleep(nanoseconds: 800_000_000)
        if process.isRunning {
            managedServerDetail = "The server did not exit immediately."
        }
        if !process.isRunning {
            managedServerDidExit()
        }
    }
    ...
    private func managedServerDidExit() {
        managedServerProcess = nil
        managedServerStandardOutput = nil
        managedServerStandardError = nil
        if managedServerState != "Failed to start" {
            managedServerState = "Stopped"
            managedServerDetail = "The Bun server is not running."
            statusMessage = managedServerDetail
            lastAction = "Managed MCP server stopped"
        }
    }

    private func appendManagedServerOutput(text: String, stream: String) {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        let prefix = stream == "stderr" ? "[server:stderr]" : "[server:stdout]"
        let lines = trimmed.split(separator: "\n", omittingEmptySubsequences: true).map(String.init)
        for line in lines {
            logLines.append("\(prefix) \(line)")
        }
        if logLines.count > 250 {
            logLines = Array(logLines.suffix(250))
        }
    }

    private func configuredServerBunPath() -> String {
        ((UserDefaults.standard.string(forKey: "pokeclaw.serverBunPath")?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap { $0.isEmpty ? nil : $0 }) ?? "bun"
    }

    private func configuredServerScriptPath() -> String {
        ((UserDefaults.standard.string(forKey: "pokeclaw.serverScriptPath")?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap { $0.isEmpty ? nil : $0 }) ?? "~/pokeclaw/server.ts"
    }

    private func configuredServerRoots() -> String {
        ((UserDefaults.standard.string(forKey: "pokeclaw.serverRoots")?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap { $0.isEmpty ? nil : $0 }) ?? "~"
    }

    private func configuredServerToken() -> String {
        UserDefaults.standard.string(forKey: "pokeclaw.serverToken") ?? ""
    }

    private func configuredServerLogLevel() -> String {
        ((UserDefaults.standard.string(forKey: "pokeclaw.serverLogLevel")?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap { $0.isEmpty ? nil : $0 }) ?? "info"
    }
    ...
    private func normalizedVersion(_ value: String) -> [Int] {
        ...
        let numeric = cleaned.split(whereSeparator: { !$0.isNumber && $0 != "." }).first.map(String.init) ?? cleaned
        return numeric.split(separator: ".").compactMap { Int($0) }
    }

    private func selectRelease(from releases: [GitHubRelease]) -> GitHubRelease? {
        let currentVersion = (Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "").lowercased()
        if currentVersion.contains("beta") {
            return releases.first(where: { $0.prerelease }) ?? releases.first
        } else {
            return releases.first(where: { !$0.prerelease }) ?? releases.first
        }
    }
