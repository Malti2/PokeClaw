...
import { bold, cyan, dim, green, yellow } from "../tui/ansi.js";

function cliEntry(): { exec: string; args: string[] } {
  // Path to this CLI's entrypoint (dist/cli.js when built).
...
export function runInstallService(): void {
  ...
  if (platform() === "darwin") {
    const plistPath = join(home, "Library", "LaunchAgents", "com.pokeclaw.plist");
